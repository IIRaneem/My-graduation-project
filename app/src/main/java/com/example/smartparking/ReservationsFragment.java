...
public class ReservationsFragment extends Fragment {
                        ...
                        if (res != null) {
                            ...
                            if (!"منتهي".equals(currentStatus) && res.getStartTimestamp() > 0 && res.getEndTimestamp() > 0) {
                                ...
                                if (!newStatus.equals(currentStatus)) {
                                    ...
                                    if ("نشط".equals(newStatus)) {
                                        updateParkingSlotStatus(res.getParkingName(), res.getSpotId(), "reserved");
                                    } else if ("منتهي".equals(newStatus)) {
                                        updateParkingSlotStatus(res.getParkingName(), res.getSpotId(), "available");
                                    }
                                }
                            }
    ...
    private void updateParkingSlotStatus(String parkingName, String spotId, String status) {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                if (snapshot.exists()) {
                    for (DataSnapshot pSnap : snapshot.getChildren()) {
                        ...
                        dbRef.child(realParkingId).child("regions").child(rStr).child("slots").child(spotId).child("status").setValue(status);
                        break;
                    }
                }
            }
